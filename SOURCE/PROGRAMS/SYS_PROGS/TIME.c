...
BOOL CREATE_ARGS() {
    RTC_DATE_TIME time = GET_DATE_TIME();
    U8 buf[100];

    // Convert numbers to strings and duplicate them
    ...
    ITOA(time.weekday, buf, 10);
    args[3] = STRDUP(buf);

    ITOA(time.day_of_month, buf, 10);
    args[4] = STRDUP(buf);
    ...
    ITOA(time.century, buf, 10);
    args[7] = STRDUP(buf);

    // Strings for weekdays/months
...
U32 main(U32 argc, PPU8 argv) {
    ...
    for(U32 i = 1; i < argc; i++) {
        if(STRCMP(argv[i], "-h") == 0 || STRCMP(argv[i], "--help") == 0) {
            printf("Usage: %s [FORMAT STRING|ARGS]\n", argv[0]);
            for(U32 j = 0; j < CONFS_LEN; j++) {
    ...
    if(!format_arg_specified) {
        ...
        if(use_conf) {
            ...
            if(atrc_format && atrc_format[0] != '\0') {
                STRNCPY(format, atrc_format, sizeof(format));
            }
            DESTROY_ATRCFD(fd);
        }
    }
