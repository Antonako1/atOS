...
void PRIC_INIT_GRAPHICAL() {
    ...
    #ifdef RUNTIME_ATGL
    msg = CREATE_PROC_MSG(KERNEL_PID, PROC_MANUAL_FLUSH_FRAMEBUFFER, NULL, 0, 0);
    SEND_MESSAGE(&msg);
    #endif
...
BOOLEAN START_PROCESS(
    ...
    sc->argc = argc;
    PROC_MESSAGE msg;
    msg = CREATE_PROC_MSG_RAW(KERNEL_PID, PROC_MSG_CREATE_PROCESS, sc, sizeof(RUN_BINARY_STRUCT), 0xDEADBEEF);
    SEND_MESSAGE(&msg);
    return TRUE;
}
...
VOID SYS_SHUTDOWN() {
    SYSCALL0(SYSCALL_SHUTDOWN_MACHINE);
}
