...
SYSCALL_ENTRY(SYSCALL_GET_KB_MOUSE_DATA, SYS_GET_KB_MOUSE_DATA) // PS2_KB_MOUSE_DATA *()
SYSCALL_ENTRY(SYSCALL_CHAR_TO_KEYCODE, SYS_CHAR_TO_KEYCODE) // U32(U8 char c))
...
SYSCALL_ENTRY(SYSCALL_KHEAP_GET_INFO, SYS_KHEAP_GET_INFO) // KHeap* (void)
SYSCALL_ENTRY(SYSCALL_KHEAP_GET_X_BLOCK, SYS_KHEAP_GET_X_BLOCK) // KHeapBlock* (U32 index)
...
SYSCALL_ENTRY(SYSCALL_VBE_DRAW_FILLED_ELLIPSE, SYS_VBE_DRAW_FILLED_ELLIPSE) // void(U32 x, U32 y, U32 rx, U32 ry, U32 color)
...
SYSCALL_ENTRY(SYSCALL_VBE_DRAW_FILLED_RECTANGLE, SYS_VBE_DRAW_FILLED_RECTANGLE) // void(U32 x, U32 y, U32 width, U32 height, U32 color)
// SYSCALL_ENTRY(SYSCALL_VBE_DRAW_TRIANGLE, SYS_VBE_DRAW_TRIANGLE) // void(U32 x1, U32 y1, U32 x2, U32 y2, U32 x3, U32 y3, U32 color)
// SYSCALL_ENTRY(SYSCALL_VBE_DRAW_FILLED_TRIANGLE, SYS_VBE_DRAW_FILLED_TRIANGLE) // void(U32 x1, U32 y1, U32 x2,
...
SYSCALL_ENTRY(SYSCALL_FILE_WRITE, SYS_FILE_WRITE) // (FAT_LFN_ENTRY *entry, const U8 *data, U32 size);
SYSCALL_ENTRY(SYSCALL_FILE_APPEND, SYS_FILE_APPEND) // (FAT_LFN_ENTRY *entry, const U8 *data, U32 size);
...
SYSCALL_ENTRY(SYSCALL_FAT_FLUSH, SYS_FAT_FLUSH) // BOOL ()
SYSCALL_ENTRY(SYSCALL_FAT_COMMIT, SYS_FAT_COMMIT) // BOOL ()
SYSCALL_ENTRY(SYSCALL_FAT_FREE_CHAIN, SYS_FAT_FREE_CHAIN) // BOOL (U32 start_cluster)
SYSCALL_ENTRY(SYSCALL_FAT_TRUNCATE_CHAIN, SYS_FAT_TRUNCATE_CHAIN) // BOOL (U32 start_cluster, U32 new_size_clusters)
SYSCALL_ENTRY(SYSCALL_FAT_CLUSTER_TO_LBA, SYS_FAT_CLUSTER_TO_LBA) // U32 (U32 cluster)
SYSCALL_ENTRY(SYSCALL_FAT_GET_NEXT_CLUSTER, SYS_FAT_GET_NEXT_CLUSTER) // U32 (U32 cluster)

// Directory entry utilities
SYSCALL_ENTRY(SYSCALL_FILE_GET_SIZE, SYS_FILE_GET_SIZE) // U32 (DIR_ENTRY *entry)
SYSCALL_ENTRY(SYSCALL_DIR_ENTRY_IS_FREE, SYS_DIR_ENTRY_IS_FREE) // BOOL (DIR_ENTRY *entry)
SYSCALL_ENTRY(SYSCALL_DIR_ENTRY_IS_DIR, SYS_DIR_ENTRY_IS_DIR) // BOOL (DIR_ENTRY *entry)
SYSCALL_ENTRY(SYSCALL_FIND_DIR_ENTRY_BY_CLUSTER, SYS_FIND_DIR_ENTRY_BY_CLUSTER) // BOOL (U32 cluster, DIR_ENTRY *out)

// Serial ports
SYSCALL_ENTRY(SYSCALL_SERIAL_WRITE_BYTE, SYS_SERIAL_WRITE_BYTE) // VOID SERIAL_WRITE_BYTE(U16 port, U8 data);
SYSCALL_ENTRY(SYSCALL_SERIAL_WRITE_DATA, SYS_SERIAL_WRITE_DATA) // VOID SERIAL_WRITE_DATA(U16 port, PU8 data, U32 len);
SYSCALL_ENTRY(SYSCALL_SERIAL_READ_BYTE, SYS_SERIAL_READ_BYTE) // VOID SERIAL_READ_BYTE(U16 port, PU8 data);
SYSCALL_ENTRY(SYSCALL_SERIAL_READ_STRING, SYS_SERIAL_READ_STRING) // VOID SERIAL_READ_STRING(U16 port, PU8 buffer, U32 max_len);
SYSCALL_ENTRY(SYSCALL_SERIAL_READ_BUFFER, SYS_SERIAL_READ_BUFFER) // VOID SERIAL_READ_BUFFER(U16 port, PU8 buffer, U32 max_len);
SYSCALL_ENTRY(SYSCALL_SERIAL_READ_WHOLE_BUFFER_HEAP, SYS_ERIAL_READ_WHOLE_BUFFER_HEAP) // PU8 SERIAL_READ_WHOLE_BUFFER_HEAP(U16 port, U32* out_len);
SYSCALL_ENTRY(SYSCALL_SERIAL_READ_WHOLE_STRING_HEAP, SYS_ERIAL_READ_WHOLE_STRING_HEAP) // PU8 SERIAL_READ_WHOLE_STRING_HEAP(U16 port, U32* out_len);
...
SYSCALL_ENTRY(SYSCALL_AC97_PLAY8, SYS_AC97_PLAY8) // BOOLEAN AC97_PLAY8(const U8* pcm, U32 frames)
SYSCALL_ENTRY(SYSCALL_AC97_PLAY16, SYS_AC97_PLAY16) // BOOLEAN AC97_PLAY16(const U16* pcm, U32 frames)
SYSCALL_ENTRY(SYSCALL_AC97_IS_PLAYING, SYS_AC97_IS_PLAYING) // BOOLEAN AC97_IS_PLAYING(void)
SYSCALL_ENTRY(SYSCALL_AC97_PAUSE,      SYS_AC97_PAUSE)      // VOID AC97_PAUSE(BOOL)
SYSCALL_ENTRY(SYSCALL_AC97_IS_PAUSED,  SYS_AC97_IS_PAUSED)  // BOOL AC97_IS_PAUSED(void)
SYSCALL_ENTRY(SYSCALL_AC97_GET_FRAME_POS, SYS_AC97_GET_FRAME_POS) // U32 AC97_GET_FRAME_POS(void)
SYSCALL_ENTRY(SYSCALL_AC97_GET_8BIT_FRAME_POS, SYS_AC97_GET_8BIT_FRAME_POS) // U32 AC97_GET_8BIT_FRAME_POS(void)
SYSCALL_ENTRY(SYSCALL_AC97_GET_VIZ, SYS_AC97_GET_VIZ) // BOOLEAN AC97_GET_VIZ(U32* bands, U32 n)

/*+++
System/Misc
---*/ 
SYSCALL_ENTRY(SYSCALL_RESTART_MACHINE, SYS_RESTART_MACHINE)
SYSCALL_ENTRY(SYSCALL_SHUTDOWN_MACHINE, SYS_SHUTDOWN_MACHINE)
SYSCALL_ENTRY(SYSCALL_GET_TIME, SYS_GET_TIME) // RTC_DATE_TIME*(void). Must be freed by caller

SYSCALL_ENTRY(SYSCALL_NULL, SYS_NULL) // U32(void) - always returns 0, for testing and placeholder purposes